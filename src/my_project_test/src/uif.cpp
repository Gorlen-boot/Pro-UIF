...
void callback(const my_project_test::can_out_MT4000::ConstPtr &input)
{
    canToUiMsg_t My_CTU;
...
void* pth_callback(void* arg)
{
    ...
    if(CurChannel.compare("ROS")==0)
    {
        ...
        ros::Publisher  my_pub = n.advertise<my_project_test::ctr_can_MT4000>("ros_to_can_MT4000",200);
        //Topic you want to subscribe
        ros::Subscriber my_sub = n.subscribe("can_to_ros_MT4000",200,callback);
        while(ros::ok()) 
        {
            if(Update_Flag==1)
            {
...
int getVehicleSupported(vehileSupported_t *vMap)
{
    ...
    {
        strtemp=vechleMode[i];
        vt.dump_bed_mode=dumpmode[i];
        vMap->insert(pair<string,vehileAttr_t>(strtemp,vt));
    }
    return VNUM;
}
...
{      
    ...
    else if(vehicleModel.compare("test1")==0) 
    {
        My_vehicleModel=vehicleModel;
        return 1;
    }
